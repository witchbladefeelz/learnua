  ...
  return (
    <div className={`relative ${className}`}>
      <button
        ...
        aria-label={!shouldShowLabels && currentLanguage ? currentLanguage.name : undefined}
        type="button"
      >
        {currentLanguage && (
          <Emoji
            symbol={currentLanguage.flag}
            decorative={shouldShowLabels}
            label={`${currentLanguage.name} flag`}
            className="text-xl"
          />
        )}
        {shouldShowLabels && currentLanguage && (
        ...
        <>
          ...
          <div
            className={`absolute right-0 ${
              variant === 'compact' ? 'mt-3' : 'mt-2'
            } py-2 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 z-20`}
          >
